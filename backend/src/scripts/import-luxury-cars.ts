...
async function importLuxuryCars() {
  try {
    console.log('🔄 开始导入豪华和欧系品牌汽车数据...\n');
    ...
    const jsonPath = path.join(__dirname, 'data', 'vietnam_cars_luxury_brands.json');
    
    if (!fs.existsSync(jsonPath)) {
      console.log('❌ 数据文件不存在:', jsonPath);
      console.log('📝 请先运行: python3 vietnam-cars-luxury-brands.py');
      process.exit(1);
    }
